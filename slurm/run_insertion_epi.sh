...
#SBATCH --job-name=s_epi_insertion_ncbi
#SBATCH --mail-user=george.bouras@adelaide.edu.au
#SBATCH --mail-type=BEGIN
#SBATCH --mail-type=END
#SBATCH --mail-type=FAIL
#SBATCH --err="s_epi_insertion_ncbi.err"
#SBATCH --output="s_epi_insertion_ncbi.out"

# Resources allocation request parameters
#SBATCH -p batch
#SBATCH -N 1              	                                # number of tasks (sequential job starts 1 task) (check this if your job unexpectedly uses 2 nodes)
#SBATCH -c 4              	                                # number of cores (sequential job calls a multi-thread program that uses 8 cores)
#SBATCH --time=04:00:00                                         # time allocation, which has the format (D-HH:MM), here set to 1 hou                                           # generic resource required (here requires 1 GPUs)
...
python3 run_pipeline.py -c 1 -f /hpcfs/users/a1667917/s_aureus/s_epi_fastas/ -o /hpcfs/users/a1667917/s_aureus/S_Epi_Insertion_NCBI_Out -p $PROF_DIR/s_aureus
